...
res.json({token});
 ...
 /*exports.registerlist = async(req,res)=>{
    try{
        const users = await User.find(); // busca todos os livros com o metódo find
        res.status(200).json(users) // retorna a lista de livros
    }catch(error){
        res.status(500).json({message: 'Erro ao buscar os usuarios',error}) // retorna o erro se houver
    }
    }
    */
